...
def _linear_search(known_encodings, query_encodings, known_names, threshold):
    names = []
    ...
    for encoding in query_encodings:
        ...
        #print(sorted(counts, key=counts.get, reverse=True)[:3])
        names.append(name)
...
class FaceRec:
    ...
    def getAllFacesInImage(image_rgb, detection_method, use_fastnn, known_encodings,
                           known_encodings_structure, known_names, threshold):
        """
        ...
        if encodings:
            if use_fastnn or known_encodings_structure == constants.ENC_KDTREE:
                ...
                names = _linear_search(known_encodings, encodings, known_names, threshold)
